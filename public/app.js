...
generateRecipe: (ingredients, details) => API.request('/api/recipe/generate', {method: 'POST',
...
if (/следующий|дальше|далее|вперёд/i.test(text)) {
  ...
  RecipeManager.prev();} else if (/первый|начало/i.test(text)) {
...
if (this.step > 0) {
this.step--;
this.render();Voice.speak(this.current.steps[this.step]);
}
}
};
// ===== STATE =====
...
tg?.HapticFeedback?.notificationOccurred('success');} catch (e) {
...
if (!file) return alert('Выберите файл');try {
...
if (days.length < 3) {
  ...
  return weekDays.map((title, i) => ({
    title,    content: allLines.slice(i * chunk, (i+1) * chunk).join('\n')
  }));
}
...
if (btnNext) {  if (this.current === total - 1) {
...
// Парсим и отображаем первый деньWeekMenu.load(data.menu.replace(/<[^>]+>/g, '\n').replace(/\n{3,}/g, '\n\n'));
...
tempDiv.style.cssText = [
  ...
  'box-sizing:border-box', 'z-index:-1'].join(';');
...
if (isAndroid) {
  ...
  if (win) {
    win.document.write(`<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Меню на неделю</title><style>body{margin:0;background:#1a1a1a;display:flex;flex-direction:column;align-items:center;padding:16px;}img{max-width:100%;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.4);}p{color:#fff;font-family:sans-serif;font-size:14px;margin-top:16px;text-align:center;opacity:0.8;}</style></head><body><img src="${dataUrl}" alt="Меню на неделю"><p>Удержи палец на картинке → «Сохранить изображение»</p></body></html>`);
    win.document.close();
  }
...
printWindow.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Меню на неделю</title><style>body{font-family:Arial,sans-serif;padding:24px;line-height:1.75;color:#2c2420;max-width:700px;margin:0 auto;}.hdr{text-align:center;padding:20px;background:linear-gradient(135deg,#d98f78,#c4735a);color:#fff;border-radius:10px;margin-bottom:24px;}.hdr h1{margin:0;font-size:22px;}.hdr p{margin:4px 0 0;font-size:13px;opacity:.9;}pre{white-space:pre-wrap;font-family:inherit;font-size:14px;}@media print{body{padding:0;}.hdr{border-radius:0;}}</style></head><body><div class="hdr"><h1>Меню на неделю</h1><p>Шеф-Повар AI · ${new Date().toLocaleDateString('ru-RU',{day:'numeric',month:'long',year:'numeric'})}</p></div><pre>${text}</pre><script>window.onload=function(){window.print();}</script></body></html>`);
...
// ===== TOAST (если ещё нет) =====function toast(message, type = 'success') {
const existing = document.querySelector('.toast-msg');
if (existing) existing.remove();
const el = document.createElement('div');
el.className = `toast-msg toast-${type}`;
el.textContent = message;
el.style.cssText = `position: fixed; bottom: 30px; left: 50%; transform: translateX(-50%); background: linear-gradient(135deg, #d98f78 0%, #c4735a 100%); color: white; padding: 14px 24px; border-radius: 16px; font-weight: 600; z-index: 9999; box-shadow: 0 8px 32px rgba(196,115,90,0.35); animation: slideUpToast 0.3s;`;
...
style.textContent = `@keyframes slideUpToast { from { transform: translate(-50%, 100px); opacity: 0; } to { transform: translate(-50%, 0); opacity: 1; } }`;
...
document.getElementById('stat-expires').textContent = data.subscription? new Date(data.subscription.expires_at).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit' })
: '—';
} catch (e) {
...
const plan = window._userPlan || 'FREE';// Таймер — только PRO+
