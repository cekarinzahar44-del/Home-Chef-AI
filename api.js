...
// Позитивная проверка «это точно про еду» — по продуктам и блюдам.
// Если сработала — гейт NOT_FOOD не включаем (защита от ложных отказов на списке продуктов).
function looksLikeFood(text) {
  const t = (text || '').toLowerCase();
  const words = [
    // действия/типы блюд
    'рецепт', 'приготов', 'сготов', 'блюд', 'суп', 'борщ', 'щи', 'салат', 'соус', 'десерт',
    'торт', 'пирог', 'пирож', 'выпеч', 'запекан', 'запеч', 'жар', 'вар', 'туш', 'гриль',
    'котлет', 'ингредиент', 'ужин', 'обед', 'завтрак', 'перекус', 'закус', 'гарнир', 'каша',
    'паста', 'пицц', 'блин', 'оладь', 'сырник', 'плов', 'рагу', 'гуляш', 'запеканк', 'омлет',
    'рол', 'суши', 'бургер', 'шаурм', 'пюре', 'вареник', 'пельмен', 'манты', 'хачапур', 'лаваш',
    // продукты
    'кури', 'куриц', 'куро', 'говядин', 'свинин', 'баранин', 'индейк', 'мяс', 'фарш', 'бекон',
    'колбас', 'сосиск', 'ветчин', 'сард', 'рыб', 'лосос', 'сёмг', 'семг', 'тунец', 'минтай',
    'сельд', 'треск', 'горбуш', 'скумбр', 'креветк', 'кальмар', 'краб', 'икр',
    'яйц', 'яиц', 'яйка', 'молок', 'кефир', 'смет', 'творог', 'сыр', 'сливк', 'йогурт', 'ряженк',
    'масл', 'маргарин', 'картоф', 'картош', 'морков', 'лук', 'чеснок', 'помидор', 'томат',
    'огурц', 'огурч', 'капуст', 'перц', 'перец', 'баклажан', 'кабач', 'тыкв', 'свёкл', 'свекл',
    'редис', 'редьк', 'сельдер', 'брокколи', 'цветн', 'горош', 'кукуруз', 'фасол', 'нут', 'чечевиц',
    'гриб', 'шампинь', 'вешенк', 'рис', 'греч', 'булгур', 'кинв', 'киноа', 'овсян', 'перлов', 'манк',
    'макарон', 'вермишел', 'спагетти', 'лапш', 'мук', 'дрожж', 'хлеб', 'батон', 'булк', 'сухар',
    'сахар', 'соль', 'мёд', 'мед ', 'варень', 'джем', 'шоколад', 'какао',
    'яблок', 'банан', 'груш', 'апельсин', 'мандарин', 'лимон', 'лайм', 'киви', 'виноград', 'ананас',
    'персик', 'абрикос', 'слив', 'вишн', 'черешн', 'клубник', 'малин', 'смородин', 'черник', 'ягод',
    'орех', 'арахис', 'миндал', 'фундук', 'семечк', 'изюм', 'кураг', 'финик', 'чернослив',
    'овощ', 'фрукт', 'зелен', 'укроп', 'петрушк', 'кинз', 'базилик', 'шпинат', 'руккол', 'салат ',
    'специ', 'припр', 'паприк', 'куркум', 'имбир', 'кориц', 'ванил', 'уксус', 'горчиц', 'майонез',
    'кетчуп', 'тесто', 'бульон', 'консерв', 'тофу', 'авокадо', 'маслин', 'оливк',
    // напитки — это тоже еда (доступ к ним ограничивает отдельный тарифный гейт)
    'напит', 'смузи', 'коктейл', 'лимонад', 'компот', 'морс', 'квас', 'кисель', 'фреш',
    'чай', 'кофе', 'глинтвейн', 'мохито', 'латте', 'капучино', 'коктейль'
  ];
  return words.some(w => t.includes(w));
}

function buildPrompt(requestType, ingredients, details, planType, prefs = {}, cookware = '', gate = true) {
  ...
  // Гейт темы включаем только для неоднозначных запросов. Для явной еды (список
  // продуктов, название блюда) он выключен — иначе Lite иногда ложно отвечает NOT_FOOD.
  const gateBlock = gate ? `

‼️ СНАЧАЛА ПРОВЕРЬ ТЕМУ. Если запрос ЯВНО не про еду (погода, новости, политика, перевод, код, математика и т.п.) — ответь РОВНО одной строкой и больше ничем: NOT_FOOD
ВАЖНО: список продуктов (например «яйца, помидоры, сыр») или название блюда — ЭТО про еду, обязательно готовь из них рецепт и НЕ отвечай NOT_FOOD.` : '';

  const system = `${allergyBlock(allergies)}Ты — шеф-повар и наставник. Пиши рецепт так, чтобы по нему даже новичок приготовил блюдо идеально с первого раза: каждый шаг понятен, ничего не нужно додумывать.${gateBlock}
...
${RECIPE_CLARITY_RULES}${cookwareNote}`;
  ...
  const gateReminder = gate ? 'Если запрос выше ЯВНО не про еду — ответь только: NOT_FOOD. Иначе ' : '';
  const reminder = `\n\n${gateReminder}ПЕРЕД ОТВЕТОМ ПРОВЕРЬ: указаны размер посуды и объём воды; каждый ингредиент доведён до конца (что достать/нарезать/вернуть); названия продуктов простые магазинные; минимум 6 шагов со временем и критерием готовности. Пиши строго по формату.`;
...
router.post('/recipe/generate', async (req, res) => {
  try {
    ...
    // ТЕМА. Если распознали еду (продукты/блюдо) — точно готовим, гейт не нужен.
    // Иначе: явное не-еда → быстрый отказ; неоднозначное → включаем ИИ-гейт.
    const isFood = looksLikeFood(ingredients) || detectDrink(ingredients);
    if (!isFood && looksNonFood(ingredients)) {
    ...
    // Гейт NOT_FOOD включаем только для неоднозначных запросов (не распознанных как еда)
    const prompt = buildPrompt(requestType, ingredients, details, planType, userPrefs, safeCookware, !isFood);

    let recipe = await callGigaChat(prompt.system, prompt.user, 2500, 0.6);
    recipe = cleanHtml(recipe);

    // ИИ распознал, что запрос не про еду — вежливый отказ.
    // Проверяем только когда гейт был включён (для явной еды он выключен).
    const trimmedRecipe = recipe.trim().replace(/<\/?b>/gi, '').trim();
    if (!isFood && /^NOT[_\s]?FOOD/i.test(trimmedRecipe)) {
      return res.status(422).json({ error: 'not_food', message: 'Я помогаю только с рецептами, переформулируйте запрос' });
    }
